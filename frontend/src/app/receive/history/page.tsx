...
export default function ReceiveHistoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async (pageNum: number) => {
      setIsLoading(true)
      try {
        const response = await apiRequest(`/api/received-posts/?page=${pageNum}&per_page=${perPage}`)
        if (response.ok) {
      ...
      <main className="relative min-h-screen px-4 py-12 bg-gradient-to-br from-[#fff1d7] via-[#ffe7f7] to-[#dff6ff] overflow-hidden text-slate-900">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          >
            ...
            <p className="text-slate-600 mt-2">気に入った曲をもう一度聴こう</p>
          </motion.div>
                ...
                {items.map((item) => (
                  ...
                  >
                    <div className="flex-shrink-0">
                      <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-amber-400 via-pink-500 to-sky-500 text-white flex items-center justify-center text-2xl shadow-md">
                        <IoMusicalNotes />
                      </div>
                    </div>
                    <div className="flex-1 space-y-1">
                      ...
                      <p className="text-sm text-slate-600">from {item.from_username}</p>
                      <div className="flex flex-wrap gap-2">
                        {(item.genres && item.genres.length ? item.genres : [item.track.primary_genre]).map((genre) => (
                          <span
                            key={genre.id}
                            className="px-3 py-1 rounded-full bg-gradient-to-r from-amber-100 via-pink-100 to-sky-100 border border-white/60 text-xs font-semibold text-slate-800"
                          >
                            {genre.name}
                          </span>
                        ))}
                      ...
                      <p className="text-xs text-slate-400">
                        受信日時: {new Date(item.received_at).toLocaleString('ja-JP')}
                      </p>
                    </div>
                    <div className="flex-shrink-0 flex gap-2">
                      <button
                        onClick={() => handleOpen(item.track.url)}
                        className="px-4 py-2 rounded-full bg-gradient-to-r from-amber-400 via-pink-500 to-sky-500 text-white text-sm font-semibold shadow-md hover:shadow-lg transition-all flex items-center gap-2"
                      >
                        <FaPlay />
                        再生
                      </button>
                ...
                <div className="flex items-center justify-between pt-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={page <= 1}
                      className="px-4 py-2 rounded-full bg-white text-slate-800 border border-amber-100 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-md transition-all text-sm font-semibold"
                    >
                      前へ
                    </button>
                    <button
                      ...
                      className="px-4 py-2 rounded-full bg-gradient-to-r from-amber-400 via-pink-500 to-sky-500 text-white shadow-sm disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-md transition-all text-sm font-semibold"
                    >
                      次へ
                    </button>
                  </div>
